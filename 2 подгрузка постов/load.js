...
function insertPosts() {
    ...
    let postsMarkup = '';
    for(let i = 0; i < scrollCheck.getBoundingClientRect().top; ++i){
        getPostMarkup(i);
        postsMarkup = postsMarkup + getPostMarkup();
        console.log(i + " :i");
        return scrollCheck.insertAdjacentHTML('beforebegin',postsMarkup);
...
function getPostMarkup(counter) {
    ...
    */
    let time = new Date();
    let num = time.getTime();
   // num = num + counter;
    console.log(num + " :num");

    return `<div class="post">
        <img src="https://picsum.photos/200/300?random=${num}" alt="${num}">
