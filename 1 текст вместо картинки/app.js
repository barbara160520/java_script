...
function handleClick(clickedButtonEvent) {
    ...
    // const cardNode = ;
    const cardNode = clickedButtonEvent.currentTarget.parentElement;
    ...
    const card = {
        ...
        button: cardNode.lastElementChild, // здесь button, который внутри .product
    };
console.log(card);
    // 4. получаем текст на кнопке, которая внутри .product
    let {button} = card;
    
    if (button.innerText =="Подробнее") { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (button.innerText =="Отмена") { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    card.img.insertAdjacentHTML('afterend',`<div class="desc">${text}</div>`);
