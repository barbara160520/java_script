...
function getProductMarkup(prod) {
    for (let el of prod){
        document.querySelector(".products").insertAdjacentHTML('beforeend', `<div>${el.name}</div> +
        + <img src="${el.imageUrl}" alt=""> <div>${el.price}</div> <a href="https://example.com/producs/${el.id}">Подробнее</a> `);
        console.log(el);
    }
}
